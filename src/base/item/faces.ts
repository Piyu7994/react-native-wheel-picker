 ...
 */
const calcHeight = (degree: number, itemHeight: number) => itemHeight * Math.cos(degToRad(degree));
...
export const createFaces = (itemHeight: number, visibleCount: number): Faces[] => {
  if (__DEV__) {
  ...
  const getScreenHeightsAndOffsets = <T extends readonly number[],>(degrees: T): [T, T] => {
    const screenHeights = degrees.map(deg => calcHeight(deg, itemHeight)) as unknown as T;
